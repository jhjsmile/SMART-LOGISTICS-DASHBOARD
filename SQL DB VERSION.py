...
def draw_v17_optimized_log(line_key, ok_btn_txt="완료 처리"):
    ...
    for idx, row in f_df.sort_values('시간', ascending=False).iterrows():
        ...
        with r_row[5]:
            ...
            else: st.markdown("<span style='color:green'>🟢 정상 완료</span>", unsafe_allow_html=True)
...
if st.session_state.current_line == "조립 라인":
    ...
    pg_title_txt = "🔍 품질 검사 공정" if st.session_state.current_line == "검사 라인" else "🚚 출하 포장"
    ...
    if not db_rep_source.empty:
        m_cols = st.columns(4)
        m_cols[0].metric("총 투입", f"{len(db_rep_source)} EA")
        m_cols[1].metric("생산 완료", f"{len(db_rep_source[(db_rep_source['라인']=='포장 라인') & (db_rep_source['상태']=='완료')])} EA")
        m_cols[2].metric("재공(WIP)", f"{len(db_rep_source[db_rep_source['상태']=='진행 중'])} EA")
        m_cols[3].metric("품질 이슈", f"{len(db_rep_source[db_rep_source['상태'].str.contains('불량', na=False)])} 건")
    ...
    if wait_list.empty: st.success("품질 이슈가 없습니다.")
    else:
        for idx, row in wait_list.iterrows():
            with st.container(border=True):
                st.write(f"**S/N: {row['시리얼']}** ({row['라인']})")
                r1c1, r1c2 = st.columns(2)
                v_cause = r1c1.text_input("원인", key=f"rc_{idx}")
                v_action = r1c2.text_input("조치", key=f"ra_{idx}")
                v_img = st.file_uploader("사진", type=['jpg','png'], key=f"ri_{idx}")
                if st.button("수리 확정", key=f"rb_{idx}", type="primary"):
                    if v_cause and v_action:
                        ...
                        db_bad.at[idx, '증상'], db_bad.at[idx, '수리'] = v_cause, v_action + (f" [사진: {url}]" if "http" in url else "")
    ...
    if not hist_df.empty: st.dataframe(hist_df[['시간', '라인', '모델', '시리얼', '증상', '수리', '작업자']], use_container_width=True, hide_index=True)
    else: st.info("내역 없음")
    ...
    if not st.session_state.admin_authenticated:
        with st.form("admin_gate"):
            m_pw = st.text_input("암호", type="password")
        ...
        r_upw = u_c2.text_input("PW 설정")
        r_url = u_c3.selectbox("권한", list(ROLES.keys()))
        if st.button("사용자 등록 및 시트 저장"):
            if r_uid and r_upw:
