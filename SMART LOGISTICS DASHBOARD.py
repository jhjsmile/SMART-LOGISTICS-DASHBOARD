...
# =================================================================
# 2. 핵심 유틸리티 함수 (v17.8 원본 유지 및 데이터 이관 로직 추가)
# =================================================================
...
def load_realtime_ledger():
    try:
        ...
        # [V18.2 핵심 추가] 기존 데이터를 제조 2반으로 이관하는 로직
        if '반' not in df.columns:
            if not df.empty:
                df.insert(1, '반', "제조 2반") # 기존 데이터는 모두 제조 2반으로 설정
            else:
                df.insert(1, '반', "")
        elif not df.empty:
            # 반 컬럼이 비어있는 기존 행들도 제조 2반으로 채움
...
# =================================================================
# 3. 세션 상태 관리 (v17.8 원본 유지 + 반 선택 변수 추가)
# =================================================================
...
# [V18.2 핵심] 현재 선택된 제조 반 상태 관리 (기본값 제조 2반)
...
# =================================================================
# 4. 로그인 및 사이드바 계층 구조 (원본 레이아웃 유지하며 계층만 추가)
# =================================================================
...
# [V18.2 핵심 추가] 제조 반별 계층형 메뉴 구성
for group in PRODUCTION_GROUPS:
    # 현재 선택된 반의 expander는 자동으로 열어둠
...
# 리포트 및 품질 메뉴 (기존 v17.8 유지)
for p in ["리포트", "불량 공정", "수리 리포트"]:
    if p in my_allowed:
        if st.sidebar.button(f"{p}", use_container_width=True, 
...
# =================================================================
# 5. 핵심 비즈니스 로직 및 컴포넌트 (v17.8 원본 그대로 유지)
# =================================================================
...
# --- 6-2. 품질 / 포장 라인 현황 ---
    ...
    pg_title_txt = f"🔍 {st.session_state.selected_group} 품질 검사 공정 현황" if st.session_state.current_line == "검사 라인" else f"🚚 {st.session_state.selected_group} 출하 포장 현황"
...
# --- 6-3. 통합 리포트 (원본 디자인 그대로 유지) ---
    ...
    if view_group != "전체":
        db_rep_source = db_rep_source[db_rep_source['반'] == view_group]
    ...
    if not db_rep_source.empty:
        ...
        with chart_l:
            ...
            fig_bar_main = px.bar(pos_sum_df, x='라인', y='수량', color='라인', title="<b>[공정 단계별 제품 분포 현황]</b>",
...
# --- 6-4. 불량 수리 센터 (원본 디자인 그대로 유지) ---
    ...
    rep_done_today = len(db_bad_target[(db_bad_target['반'] == st.session_state.selected_group) & (db_bad_target['상태'] == "수리 완료(재투입)") & (db_bad_target['시간'].apply(check_today_match))])
        ...
        for idx, row in wait_list.iterrows():
            with st.container(border=True):
                ...
                if r2c2.button("✅ 수리 확정", key=f"rb_{idx}", type="primary", use_container_width=True):
                    ...
                    else: st.error("필수 항목(원인 및 조치내용)을 채워주세요.")

# --- 6-5. 수리 이력 리포트 (원본 디자인 그대로 유지) ---
elif st.session_state.current_line == "수리 리포트":
    st.markdown("<h2 class='centered-title'>📈 품질 분석 및 수리 이력 리포트</h2>", unsafe_allow_html=True)
    view_group_rep = st.radio("조회 범위 선택", ["전체"] + PRODUCTION_GROUPS, horizontal=True, key="repair_rep_radio")
    db_hist_ledger = st.session_state.production_db
    if view_group_rep != "전체":
        db_hist_ledger = db_hist_ledger[db_hist_ledger['반'] == view_group_rep]
    ...
    if not hist_df.empty:
        ...
        st.dataframe(hist_df[['시간', '반', '라인', '모델', '시리얼', '증상', '수리', '작업자']], use_container_width=True, hide_index=True)
    else: st.info("현재까지 기록된 품질 이슈 내역이 없습니다.")

# --- 6-6. 마스터 정보 관리 (어드민 / 원본 그대로 유지) ---
    ...
    if not st.session_state.admin_authenticated:
        with st.form("master_verify_gate"):
            ...
            if st.form_submit_button("권한 인증"):
                if m_pw_in == "admin1234": st.session_state.admin_authenticated = True; st.rerun()
        ...
        r_uid, r_upw, r_url = u_c1.text_input("ID 생성"), u_c2.text_input("PW 설정", type="password"), u_c3.selectbox("권한 부여", ["user", "admin"])
        if st.button("사용자 정보 업데이트 실행", use_container_width=True):
            if r_uid and r_upw: st.session_state.user_db[r_uid] = {"pw": r_upw, "role": r_url}; st.success(f"사용자 '{r_uid}' 계정 설정 완료"); st.rerun()
        with st.expander("현재 시스템 등록 계정 전체 리스트 확인"): st.table(pd.DataFrame.from_dict(st.session_state.user_db, orient='index'))
        st.divider()
        if st.button("⚠️ 시스템 전체 실적 데이터 영구 초기화(초기화)", type="secondary", use_container_width=True):
            st.session_state.production_db = pd.DataFrame(columns=['시간', '반', '라인', 'CELL', '모델', '품목코드', '시리얼', '상태', '증상', '수리', '작업자'])
            push_to_cloud(st.session_state.production_db); st.rerun()

# =================================================================
# [ PMS v18.2 최종 소스코드 종료 ]
# =================================================================
