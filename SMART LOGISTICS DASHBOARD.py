...
# =================================================================
# 2. 세션 상태(Session State) 초기화 (1~11번 핵심 기능 포함)
# =================================================================
...
if st.session_state.current_line == "마스터 관리":
    ...
    if not st.session_state.admin_authenticated:
        ...
        st.markdown("<div class='section-title'>📋 마스터 데이터 및 계정 관리</div>", unsafe_allow_html=True)
        m1, m2 = st.columns(2)
        with m1:
            with st.container(border=True):
                st.write("**모델/품목 추가**")
                m_add = st.text_input("신규 모델명")
        ...
        with m2:
            with st.container(border=True):
                st.write("**시스템 계정 권한 부여**")
                uid, upw = st.text_input("새 ID"), st.text_input("새 PW")
                if st.button("계정 저장", use_container_width=True):
                    st.session_state.user_db[uid] = {"pw": upw, "role": "user"}; st.rerun()

        st.divider()
        st.markdown("<div class='section-title'>📤 CSV 관리 및 DB 초기화</div>", unsafe_allow_html=True)
        up_f = st.file_uploader("마스터 데이터 CSV 업로드", type="csv")
        b1, b2, b3 = st.columns(3)
        b1.button("💾 모델 백업", use_container_width=True)
        b2.button("💾 품목 백업", use_container_width=True)
        if b3.button("⚠️ 전체 DB 초기화", type="secondary", use_container_width=True):
            st.session_state.production_db = pd.DataFrame(columns=['시간', '라인', 'CELL', '모델', '품목코드', '시리얼', '상태', '증상', '수리'])
            st.rerun()

# --- (5-2) 조립 라인 (셀 변경 시 초기화 및 상태 색상) ---
    ...
    if st.session_state.selected_cell != "전체 CELL":
        with st.container(border=True):
            ...
            # 셀 변경 시 "선택하세요." 초기화 위해 고유 key 부여
            m_choice = st.selectbox("모델 선택", ["선택하세요."] + st.session_state.master_models, key=f"m_sel_{st.session_state.selected_cell}")
    ...
    if not l_db.empty:
        ...
        for idx, row in l_db.sort_values('시간', ascending=False).iterrows():
            ...
            with lr[5]:
                ...
                elif row['상태'] == "불량 처리 중": st.error("🔴 불량 처리 중")
                else: st.success("🟢 완료")

# --- (5-3) 품질 / 포장 라인 (입고 조회 및 불량 로직 수정) ---
    ...
    if not curr_log.empty:
        for idx, row in curr_log.sort_values('시간', ascending=False).iterrows():
            ...
            with lr[4]:
                if row['상태'] in ["진행 중", "수리 완료(재투입)"]:
                    b1, b2 = st.columns(2)
                    label = "합격" if st.session_state.current_line=="검사 라인" else "완료"
                    if b1.button(label, key=f"ok_c_{idx}"): st.session_state.production_db.at[idx, '상태'] = "완료"; st.rerun()
...
# --- (5-4) 리포트 (정수 그래프) ---
    ...
    if not db.empty:
        ...
        with c_left:
            fig = px.bar(db[db['상태']=='완료'].groupby('라인').size().reset_index(name='수량'), x='라인', y='수량', color='라인', title="라인별 양품 실적")
            fig.update_layout(title_x=0.5, yaxis=dict(dtick=1, tickformat='d'))
