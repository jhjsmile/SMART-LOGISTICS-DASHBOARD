...
# =================================================================
# 1. 시스템 설정 및 권한/스타일 정의
# =================================================================
st.set_page_config(page_title="생산 통합 관리 시스템 v11.8", layout="wide")

# 권한 정의: 상업용 고도화 (권한별 메뉴 접근 제한)
ROLES = {
...
def load_data():
    try:
        # TTL=0으로 설정하여 항상 구글 시트의 최신 데이터를 읽어옴
    ...
    with l_col:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("로그인", use_container_width=True):
                # 실제 운영 시 이 계정 목록은 DB나 마스터 관리에서 호출하도록 구성 가능
                user_db = {
                    "admin": {"pw": "admin1234", "role": "admin"},
                    "manager1": {"pw": "man1234", "role": "manager"},
                    "worker1": {"pw": "1234", "role": "worker"}
                }
                if uid in user_db and user_db[uid]["pw"] == upw:
                    st.session_state.login_status, st.session_state.user_id = True, uid
                    st.session_state.user_role = user_db[uid]["role"]
                    st.rerun()
                else: st.error("계정 정보를 확인하세요.")
    st.stop()

# =================================================================
# 4. 사이드바 내비게이션 (권한별 필터링)
# =================================================================
st.sidebar.title(f"🏭 {st.session_state.user_id}님")
st.sidebar.info(f"접속권한: {st.session_state.user_role}")
...
# 권한별 메뉴만 노출 (고도화 포인트 1)
allowed_menus = ROLES.get(st.session_state.user_role, ["조립 라인"])
menu_icons = {"조립 라인":"📦", "검사 라인":"🔍", "포장 라인":"🚚", "리포트":"📊", "불량 공정":"🛠️", "수리 리포트":"📈", "마스터 관리":"🔐"}

for menu_name in allowed_menus:
    is_active = st.session_state.current_line == menu_name
    if st.sidebar.button(f"{menu_icons[menu_name]} {menu_name}", use_container_width=True, 
                         type="primary" if is_active else "secondary"):
        nav(menu_name)

# =================================================================
# 5. 지능형 알림 배너 (고도화 포인트 2)
# =================================================================
db = st.session_state.production_db
bad_count = len(db[db['상태'] == "불량 처리 중"])
if bad_count > 0:
    st.markdown(f"<div class='alarm-banner'>⚠️ 현장 알림: 현재 수리 센터에 수리 대기 중인 제품이 {bad_count}건 있습니다.</div>", unsafe_allow_html=True)

# =================================================================
# 6. 공용 컴포넌트 (v9.1 로직 복구 + 작업자 기록 추가)
# =================================================================
...
def confirm_entry_dialog():
    ...
    if c1.button("✅ 승인", type="primary", use_container_width=True):
        new_row = {
            ...
            '작업자': st.session_state.user_id # 고도화 포인트 3: 작업자 기록
        }
...
# =================================================================
# 7. 페이지별 메인 로직 (v9.1 전체 기능 + 고도화 통합)
# =================================================================
...
# --- 7-3. 통합 리포트 (고도화: 지능형 KPI + 분석) ---
elif st.session_state.current_line == "리포트":
    st.markdown("<h2 class='centered-title'>📊 통합 생산 분석 리포트</h2>", unsafe_allow_html=True)
    if st.button("🔄 최신 데이터 불러오기"): st.session_state.production_db = load_data(); st.rerun()
    
    if not db.empty:
        # 고도화 포인트: 실시간 KPI 카드
        total_p = len(db[(db['라인'] == '포장 라인') & (db['상태'] == '완료')])
        total_ng = len(db[db['상태'].str.contains("불량", na=False)])
        ftt = (total_p / (total_p + total_ng) * 100) if (total_p + total_ng) > 0 else 100
        
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("최종 생산량", f"{total_p} EA")
        m2.metric("현재 공정 중", f"{len(db[db['상태'] == '진행 중'])} EA")
        m3.metric("누적 불량", f"{total_ng} 건", delta=total_ng, delta_color="inverse")
        m4.metric("직행률(FTT)", f"{ftt:.1f}%")
        
        st.divider()
        # 시리얼 추적 기능
        with st.expander("🔍 시리얼 번호 이력 추적 (Traceability)"):
            q_sn = st.text_input("조회할 시리얼 번호")
        ...
        # 고도화: 작업자별 기여도 분석
        st.plotly_chart(px.bar(db.groupby('작업자').size().reset_index(name='건수'), x='작업자', y='건수', color='작업자', title="👷 작업자별 업무 처리 기여도"), use_container_width=True)
        st.dataframe(db.sort_values('시간', ascending=False), use_container_width=True, hide_index=True)

# --- 7-4. 불량 수리 센터 (입력 보존 + 사진 미리보기) ---
    ...
    if bad_items.empty:
        st.success("✅ 현재 수리 대기 중인 제품이 없습니다.")
    else:
        for idx, row in bad_items.iterrows():
            with st.container(border=True):
                ...
                up_img = st.file_uploader("수리 사진 미리보기 (확인용)", type=['jpg','png','jpeg'], key=f"img_{idx}")
...
# --- 7-5. 수리 리포트 (도넛 그래프 포함) ---
    ...
    if not r_db.empty:
        c1, c2 = st.columns(2)
        with c1: st.plotly_chart(px.bar(r_db.groupby('라인').size().reset_index(name='수량'), x='라인', y='수량', title="라인별 수리 발생 건수"), use_container_width=True)
        with c2: st.plotly_chart(px.pie(r_db.groupby('모델').size().reset_index(name='수량'), values='수량', names='모델', hole=0.3, title="수리 모델별 비중"), use_container_width=True)
        st.dataframe(r_db[['시간', '라인', '모델', '시리얼', '증상', '수리', '작업자']], use_container_width=True, hide_index=True)

# --- 7-6. 마스터 관리 (Admin 전용) ---
elif st.session_state.current_line == "마스터 관리":
    st.markdown("<h2 class='centered-title'>🔐 시스템 마스터 관리 (Admin)</h2>", unsafe_allow_html=True)
    if not st.session_state.admin_authenticated:
        with st.form("admin_auth"):
            apw = st.text_input("관리자 인증 PW", type="password")
            if st.form_submit_button("인증하기"):
                if apw == "admin1234": st.session_state.admin_authenticated = True; st.rerun()
                else: st.error("인증 실패")
    else:
        if st.button("🔓 관리자 세션 종료"): st.session_state.admin_authenticated = False; nav("조립 라인")
        ...
        with m1:
            with st.container(border=True):
                ...
                st.divider()
                sm = st.selectbox("품목 등록용 모델", st.session_state.master_models)
        ...
        with m2:
            with st.container(border=True):
                ...
                st.download_button("📥 전체 데이터 다운로드(CSV)", csv_data, "backup.csv", "text/csv", use_container_width=True)
                st.divider()
