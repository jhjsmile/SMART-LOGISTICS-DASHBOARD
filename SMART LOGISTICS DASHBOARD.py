...
def push_to_cloud(df, sheet_name="Sheet1"):
    try:
        ...
        st.cache_data.clear()
    except Exception as e: st.error(f"데이터 저장 실패: {e}")

# [신규] 시트의 Master_DB 탭에서 대량으로 마스터 정보 동기화
def sync_master_data():
    ...
    if not master_df.empty:
        for _, row in master_df.iterrows():
            g = str(row.get('반', '')).strip()
            mod = str(row.get('모델', '')).strip()
            it = str(row.get('품목코드', '')).strip()
...
if "마스터 관리" in allowed_nav:
    ...
    if st.sidebar.button("🔐 마스터 관리", use_container_width=True, type="primary" if m_active else "secondary"): 
...
def trigger_entry_dialog():
    ...
    if c_ok.button("✅ 입고 승인", type="primary", use_container_width=True):
        ...
        if not idx_match.empty:
            idx = idx_match[0]
            db_full.at[idx, '시간'], db_full.at[idx, '라인'], db_full.at[idx, '상태'], db_full.at[idx, '작업자'] = get_now_kst_str(), st.session_state.current_line, '진행 중', st.session_state.user_id
...
if curr_l == "현황판":
    ...
    with k4: st.markdown(f"<div class='stat-box'><div class='stat-label'>⚠️ 분석 중 불량</div><div class='stat-value' style='color:#fa5252;'>{len(db[db['상태'].str.contains('불량', na=False)])}</div></div>", unsafe_allow_html=True)
    st.divider()
    cl, cr = st.columns([1.5, 1])
    with cl:
        st.markdown("#### 📈 반별 생산 분포")
        if not db.empty:
            fig_bar = px.histogram(db, x="반", color="라인", barmode="group", template="plotly_white", color_discrete_map={"조립 라인": "#0068C9", "검사 라인": "#A0D1FB", "포장 라인": "#FFABAB"})
            ...
            if st.form_submit_button("▶️ 생산 등록 시작", use_container_width=True, type="primary"):
                if t_mod != "선택하세요." and t_sn:
                    if t_sn in db['시리얼'].values: st.error("❌ 이미 등록된 시리얼입니다.")
                        ...
                        new_row = {'시간': get_now_kst_str(), '반': curr_g, '라인': "조립 라인", '모델': t_mod, '품목코드': t_item, '시리얼': t_sn, '상태': '진행 중', '작업자': st.session_state.user_id}
...
# --- 7-2. 검사 라인 ---
    ...
    st.markdown(f"<h2 class='centered-title'>🔍 {curr_g} 검사 현황</h2>", unsafe_allow_html=True)
    with st.container(border=True):
        st.write("#### 📥 입고 대기 (조립 완료 물량)")
        wait_df = db[(db['반'] == curr_g) & (db['라인'] == "조립 라인") & (db['상태'] == "완료")]
    ...
    st.divider()
    f_df = db[(db['반'] == curr_g) & (db['라인'] == "검사 라인")]
    if not f_df.empty:
        h = st.columns([2.5, 2, 2, 2, 4])
        for col, txt in zip(h, ["기록 시간", "모델", "품목", "시리얼", "제어"]): col.write(f"**{txt}**")
        for idx, row in f_df.sort_values('시간', ascending=False).iterrows():
            ...
            with r[4]:
                if row['상태'] in ["진행 중", "수리 완료(재투입)"]:
                    b1, b2 = st.columns(2)
                    if b1.button("합격", key=f"ok_{idx}"): db.at[idx, '상태'] = "완료"; push_to_cloud(db); st.rerun()
                    if b2.button("🚫불량", key=f"ng_{idx}"): db.at[idx, '상태'] = "불량 처리 중"; push_to_cloud(db); st.rerun()
                elif row['상태'] == "불량 처리 중": st.markdown("<span class='bad-status-badge'>✅ 불량 처리 중</span>", unsafe_allow_html=True)
                else: st.write(f"✅ {row['상태']}")
...
# --- 7-3. 포장 라인 ---
    ...
    st.markdown(f"<h2 class='centered-title'>📦 {curr_g} 포장 현황</h2>", unsafe_allow_html=True)
    with st.container(border=True):
        st.write("#### 📥 입고 대기 (검사 완료 물량)")
        wait_df = db[(db['반'] == curr_g) & (db['라인'] == "검사 라인") & (db['상태'] == "완료")]
        if not wait_df.empty:
            w_cols = st.columns(4)
            for i, (idx, row) in enumerate(wait_df.iterrows()):
                if w_cols[i%4].button(f"입고: {row['시리얼']}", key=f"in_{idx}"):
    ...
    st.divider()
    f_df = db[(db['반'] == curr_g) & (db['라인'] == "포장 라인")]
    if not f_df.empty:
        h = st.columns([2.5, 2, 2, 2, 4])
        for col, txt in zip(h, ["기록 시간", "모델", "품목", "시리얼", "제어"]): col.write(f"**{txt}**")
        for idx, row in f_df.sort_values('시간', ascending=False).iterrows():
...
# --- 7-4. 리포트 ---
    ...
    st.markdown("<h2 class='centered-title'>📊 실시간 생산 리포트</h2>", unsafe_allow_html=True)
    sel_range = st.radio("조회 범위", ["전체"] + PRODUCTION_GROUPS, horizontal=True)
...
# --- 7-5. 불량 공정 (수리) ---
    ...
    if wait_b.empty: st.success("품질 이슈 없음")
        ...
        for idx, row in wait_b.iterrows():
            with st.container(border=True):
                st.write(f"**이슈 S/N: {row['시리얼']}** ({row['모델']})")
                r1, r2 = st.columns(2)
                vc, va = r1.text_input("불량 원인 판정", key=f"c_{idx}"), r2.text_input("수리 조치 내역", key=f"a_{idx}")
                img = st.file_uploader("증빙 사진 업로드", key=f"i_{idx}")
                if st.button("수리 확정 등록", key=f"b_{idx}", type="primary"):
                    if vc and va:
...
# --- 7-6. 수리 이력 ---
    ...
    if not h_df.empty:
        cl, cr = st.columns([1.5, 1])
        with cl: st.plotly_chart(px.bar(h_df.groupby('모델').size().reset_index(name='건수'), x='모델', y='건수', title="모델별 불량 발생 빈도"), use_container_width=True)
        with cr: st.plotly_chart(px.pie(h_df.groupby('증상').size().reset_index(name='건수'), values='건수', names='증상', title="주요 증상 분포"), use_container_width=True)
        st.dataframe(h_df, use_container_width=True, hide_index=True)
    else: st.info("수리 이력이 존재하지 않습니다.")

# --- 7-7. 마스터 관리 (시트 대량 연동 패치) ---
elif curr_l == "마스터 관리":
    st.markdown("<h2 class='centered-title'>🔐 마스터 대량 관리 (Sheet 연동)</h2>", unsafe_allow_html=True)
    if not st.session_state.admin_authenticated:
        with st.form("auth_f"):
            if st.form_submit_button("인증 시작") and st.text_input("비밀번호", type="password") in ["admin1234", "master1234"]:
                st.session_state.admin_authenticated = True; st.rerun()
    else:
        st.info("💡 **사용 안내:** 구글 스프레드시트의 **'Master_DB'** 탭에서 모델과 품목을 대량으로 입력한 후 아래 버튼을 누르세요.")
        ...
        st.divider()
        st.subheader("현재 시스템 등록 현황 (동기화 결과)")
        ...
        c1, c2 = st.columns(2)
        with c1: st.download_button("📥 전체 실적 CSV 백업", db.to_csv(index=False).encode('utf-8-sig'), "Backup.csv", use_container_width=True)
        with c2: 
            if st.button("⚠️ 전체 실적 초기화", type="secondary", use_container_width=True):
                st.session_state.production_db = pd.DataFrame(columns=['시간','반','라인','모델','품목코드','시리얼','상태','증상','수리','작업자'])
                push_to_cloud(st.session_state.production_db); st.rerun()

# =================================================================
# [ PMS v25.1 무생략 최종 완결판 END ]
# =================================================================
