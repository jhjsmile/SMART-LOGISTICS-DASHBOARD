...
# 1. 시스템 설정
# =================================================================
st.set_page_config(page_title="생산 통합 관리 시스템 v9.0", layout="wide")
...
st.sidebar.title(f"🏭 {st.session_state.user_id}님 ({st.session_state.user_role})")
...
# [10번 핵심] 관리자 권한일 때만 마스터 관리 메뉴 노출
...
def display_process_log(line_name, ok_label="완료"):
    st.divider()
    st.markdown(f"<h3 class='centered-title'>📝 {line_name} 로그</h3>", unsafe_allow_html=True)
    ...
    for idx, row in l_db.sort_values('시간', ascending=False).iterrows():
        ...
        with lr[5]:
            if row['상태'] in ["진행 중", "수리 완료(재투입)"]:
                b1, b2 = st.columns(2)
                if b1.button(ok_label, key=f"ok_{idx}"): st.session_state.production_db.at[idx, '상태'] = "완료"; st.rerun()
                if b2.button("🚫불량", key=f"ng_{idx}"): st.session_state.production_db.at[idx, '상태'] = "불량 처리 중"; st.rerun()
            elif row['상태'] == "불량 처리 중": st.markdown("<span class='status-red'>🔴 수리 중</span>", unsafe_allow_html=True)
            else: st.markdown("<span class='status-green'>🟢 완료</span>", unsafe_allow_html=True)

# =================================================================
# 5. 메인 공정 로직 (조립/검사/포장/리포트/수리)
# =================================================================
if st.session_state.current_line == "조립 라인":
    st.markdown("<h2 class='centered-title'>📦 조립 라인</h2>", unsafe_allow_html=True)
    ...
    line_title = "🔍 검사 현황" if st.session_state.current_line == "검사 라인" else "🚚 포장 현황"
    ...
    with st.container(border=True):
        f1, f2 = st.columns(2)
        sm = f1.selectbox("모델", ["선택하세요."] + st.session_state.master_models, key=f"sm_{st.session_state.current_line}")
        si = f2.selectbox("품목", ["품목 선택"] + st.session_state.master_items_dict.get(sm, []) if sm != "선택하세요." else ["품목 선택"], key=f"si_{st.session_state.current_line}")
    ...
    db = st.session_state.production_db
    if not db.empty:
        met = st.columns(4); met[0].metric("최종 완료", len(db[db['상태'] == '완료'])); met[1].metric("진행 중", len(db[db['상태'] == '진행 중']))
        st.divider(); c1, c2 = st.columns([3, 2])
        with c1: st.plotly_chart(px.bar(db[db['상태']=='완료'].groupby('라인').size().reset_index(name='수량'), x='라인', y='수량', title="공정별 실적"), use_container_width=True)
        with c2: st.plotly_chart(px.pie(db.groupby('모델').size().reset_index(name='수량'), values='수량', names='모델', title="모델 비중"), use_container_width=True)
    ...
    st.markdown("<h2 class='centered-title'>🛠️ 수리 센터</h2>", unsafe_allow_html=True)
    bad = st.session_state.production_db[st.session_state.production_db['상태'] == "불량 처리 중"]
    if bad.empty: st.success("✅ 수리 대기 없음")
    else:
        for idx, row in bad.iterrows():
            with st.container(border=True):
                st.write(f"**S/N: {row['시리얼']}** ({row['모델']})")
                c1, c2, c3 = st.columns([4, 4, 2])
                sv, av = c1.text_input("원인", key=f"s_{idx}"), c2.text_input("조치", key=f"a_{idx}")
                if c3.button("✅ 수리 완료", key=f"r_{idx}", disabled=not(sv.strip() and av.strip())):
                    st.session_state.production_db.at[idx, '상태'] = "수리 완료(재투입)"
                    st.session_state.production_db.at[idx, '증상'], st.session_state.production_db.at[idx, '수리'] = sv, av; st.rerun()

elif st.session_state.current_line == "수리 리포트":
    st.markdown("<h2 class='centered-title'>📈 수리 현황</h2>", unsafe_allow_html=True)
    rep_db = st.session_state.production_db[st.session_state.production_db['상태'].str.contains("재투입", na=False)]
    if not rep_db.empty: st.dataframe(rep_db[['시간', '라인', '모델', '시리얼', '증상', '수리']], use_container_width=True, hide_index=True)

# =================================================================
# 6. 마스터 관리 (10번: ID/PW 부여 및 권한 관리 + 데이터 백업)
# =================================================================
    ...
    if not st.session_state.admin_authenticated:
        ...
        # 10번 요청: 계정 생성 및 권한 부여
        st.markdown("<div class='section-title'>👤 10. 사용자 계정 관리 (ID/PW 부여)</div>", unsafe_allow_html=True)
        u_col1, u_col2, u_col3 = st.columns([3, 3, 2])
        new_uid = u_col1.text_input("신규 ID")
        new_upw = u_col2.text_input("신규 PW", type="password")
        new_role = u_col3.selectbox("권한", ["user", "admin"])
        if st.button("계정 생성/업데이트", use_container_width=True):
            if new_uid and new_upw:
                ...
                st.success(f"계정 [{new_uid}] 생성 완료!"); st.rerun()
        
        with st.expander("현재 등록된 계정 목록"):
            st.write(st.session_state.user_db)

        st.divider()
        # 데이터 백업 기능
        st.markdown("<div class='section-title'>💾 데이터 백업 및 로드</div>", unsafe_allow_html=True)
        c_dn, c_up = st.columns(2)
        with c_dn:
            csv = st.session_state.production_db.to_csv(index=False).encode('utf-8-sig')
            st.download_button("📥 전체 데이터 다운로드", csv, "backup.csv", "text/csv", use_container_width=True)
        with c_up:
            up_f = st.file_uploader("CSV 로드", type="csv")
            if up_f and st.button("📤 데이터 병합"):
                st.session_state.production_db = pd.concat([st.session_state.production_db, pd.read_csv(up_f)], ignore_index=True); st.rerun()

        st.divider()
        # 기준정보 설정
        st.markdown("<div class='section-title'>📋 생산 기준정보 설정</div>", unsafe_allow_html=True)
        m1, m2 = st.columns(2)
        with m1:
            nm = st.text_input("모델 추가")
            if st.button("모델 등록") and nm: st.session_state.master_models.append(nm); st.session_state.master_items_dict[nm]=[]; st.rerun()
        with m2:
            sm = st.selectbox("모델 선택", st.session_state.master_models)
            ni = st.text_input("품목 추가")
            if st.button("품목 등록") and ni: st.session_state.master_items_dict[sm].append(ni); st.rerun()
