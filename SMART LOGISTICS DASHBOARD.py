...
# 1. 시스템 설정 및 스타일 정의
# =================================================================
st.set_page_config(page_title="생산 통합 관리 시스템 v15.5", layout="wide")
...
def confirm_entry_dialog():
    ...
    if c1.button("✅ 승인", type="primary", use_container_width=True):
        new_row = {
            ...
            '시리얼': st.session_state.confirm_target, '상태': '진행 중', '증상': '', '수리': '', '작업자': st.session_state.user_id
        }
        ...
        with lr[5]:
            if row['상태'] in ["진행 중", "수리 완료(재투입)"]:
                ...
                if b2.button("🚫불량", key=f"ng_{idx}"):
                    ...
                    st.session_state.production_db.at[idx, '작업자'] = st.session_state.user_id
                    save_to_gsheet(st.session_state.production_db); st.rerun()
            elif row['상태'] == "불량 처리 중": st.markdown("<span style='color:red;'>🔴 불량 처리 중</span>", unsafe_allow_html=True)
            else: st.markdown("<span style='color:green;'>🟢 완료</span>", unsafe_allow_html=True)
...
# --- 6-3. 통합 리포트 [수정: 연회색 배경 + 흰색 격자] ---
    ...
    if not db.empty:
        ...
        fig1.update_layout(plot_bgcolor='#f0f2f6', paper_bgcolor='rgba(0,0,0,0)') # 배경: 연회색, 외부: 투명
        fig1.update_xaxes(showgrid=True, gridcolor='white') # 격자: 흰색
        fig1.update_yaxes(showgrid=True, gridcolor='white', rangemode='tozero') 
        ...
        fig2.update_layout(plot_bgcolor='#f0f2f6', paper_bgcolor='rgba(0,0,0,0)')
        fig2.update_xaxes(showgrid=True, gridcolor='white')
        fig2.update_yaxes(showgrid=True, gridcolor='white', rangemode='tozero')
...
# --- 6-5. 수리 리포트 [수정: 연회색 배경 + 흰색 격자] ---
    ...
    if not rep_db.empty:
        ...
        fig_r1.update_layout(plot_bgcolor='#f0f2f6', paper_bgcolor='rgba(0,0,0,0)')
        fig_r1.update_xaxes(showgrid=True, gridcolor='white')
        fig_r1.update_yaxes(showgrid=True, gridcolor='white', rangemode='tozero')
...
# --- 6-6. 마스터 관리 ---
    ...
    if not st.session_state.admin_authenticated:
        ...
        new_role_choice = u_col3.selectbox("권한 설정", ["control_tower", "assembly_team", "qc_team", "packing_team", "master"])
