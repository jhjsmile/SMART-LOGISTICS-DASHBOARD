...
if not st.session_state.auth_done:
    ...
    access_key = st.text_input("접근 인증키 (7777)", type="password")
    if st.button("접속 승인"):
        if access_key == "7777":
...
# --- [사이드바: image_5fcc3d.png 기능을 100% 그대로 복구] ---
with st.sidebar:
    st.title("🛠️ 관리자 도구")
    
    # 1. 항목 추가/삭제 (사용자님 스샷 첫 번째 메뉴)
    with st.expander("📂 항목 추가/삭제", expanded=True):
        ...
        if st.session_state.categories:
            del_cat = st.selectbox("삭제할 항목 선택", list(st.session_state.categories.keys()))
            if st.button("❌ 삭제"):
                del st.session_state.categories[del_cat]
                st.rerun()

    # 2. 시리얼 자동 생성 (사용자님 스샷 두 번째 메뉴)
    with st.expander("🔢 시리얼 자동 생성"):
        if st.session_state.categories:
            ...
            c1, c2 = st.columns(2)
            start_n = c1.number_input("시작", value=1)
            end_n = c2.number_input("끝", value=10)
            if st.button("🚀 생성 실행"):
                for i in range(int(start_n), int(end_n) + 1):
                    sn = f"{prefix}{i:03d}"
                    st.session_state.categories[target_cat][sn] = False
                st.success(f"{int(end_n-start_n+1)}개 생성 완료!")

    # 3. 데이터 내보내기/로드 (사용자님 스샷 세 번째 메뉴)
...
for i, tab in enumerate(tabs):
    with tab:
        ...
        if i < 3: # 입고, 조립, 포장
            s_input = st.text_input(f"{labels[i]} 스캔창", key=f"inp_{i}")
            if st.button("확인", key=f"btn_{i}"): process_scan(s_input, labels[i])
        else: # 불량 처리
            f_input = st.text_input("불량 시리얼 스캔", key="f_inp")
            reason = st.selectbox("사유", ["파손", "기타"], key="f_rea")
            if st.button("불량 등록"):
                if f_input:
                    cc = clean_serial(f_input)
                    save_log_to_csv(cc, "불량", f"사유:{reason}")
                    st.warning(f"⚠️ {cc} 불량 처리됨")

# 하단 현황판 (스샷의 하단 진행률 표시 복구)
st.divider()
if st.session_state.categories:
    ...
    for i, (cat, items) in enumerate(st.session_state.categories.items()):
        ...
        with cols[i]:
            st.metric(cat, f"{done}/{total}", f"{int(done/total*100) if total>0 else 0}%")
            # 오늘자 상세 로그 보기 버튼 (스샷 기능 복구)
            if st.button(f"📋 {cat} 로그", key=f"log_{i}"):
                st.write(items)
