...
# 1. 시스템 설정 및 구글 시트 연결
# =================================================================
st.set_page_config(page_title="생산 통합 관리 시스템 v10.0", layout="wide")
...
# 구글 시트 커넥션 초기화
# (Streamlit Cloud의 Secrets에 설정을 넣거나, 코드 내에서 직접 연결 가능)
conn = st.connection("gsheets", type=GSheetsConnection)

def load_data():
    try:
        # ttl="0"은 캐시를 사용하지 않고 항상 최신 데이터를 가져오겠다는 의미입니다.
...
if st.sidebar.button("전체 로그아웃"): st.session_status = False; st.rerun()
st.sidebar.divider()
...
def confirm_entry_dialog():
    ...
    if c1.button("✅ 승인", type="primary", use_container_width=True):
        ...
        update_gsheet(st.session_state.production_db)
        ...
        with lr[5]:
            if row['상태'] in ["진행 중", "수리 완료(재투입)"]:
                ...
                if b1.button(ok_label, key=f"ok_{idx}"):
                    st.session_state.production_db.at[idx, '상태'] = "완료"; update_gsheet(st.session_state.production_db); st.rerun()
                if b2.button("🚫불량", key=f"ng_{idx}"):
                    st.session_state.production_db.at[idx, '상태'] = "불량 처리 중"; update_gsheet(st.session_state.production_db); st.rerun()
                ...
                if st.form_submit_button("▶️ 조립 등록", use_container_width=True, type="primary"):
                    if m_choice != "선택하세요." and s_input:
                        if not st.session_state.production_db[(st.session_state.production_db['시리얼'] == s_input) & (st.session_state.production_db['상태'] != "완료")].empty:
                            ...
                            st.session_state.production_db = pd.concat([st.session_state.production_db, pd.DataFrame([new_row])], ignore_index=True)
                            update_gsheet(st.session_state.production_db); st.rerun()
...
# --- 통합 리포트 (KPI + 추적) ---
    ...
    if st.button("🔄 시트 데이터 새로고침"): st.session_state.production_db = load_data(); st.rerun()
    db = st.session_state.production_db
    if not db.empty:
        ...
        st.divider()
        with st.expander("🔍 시리얼 이력 추적"):
            sn_q = st.text_input("조회 시리얼")
            if sn_q: st.dataframe(db[db['시리얼'] == sn_q], use_container_width=True)
...
# --- 불량 수리 센터 (보존/미리보기) ---
    ...
    if bad.empty: st.success("✅ 수리 대기 물량이 없습니다.")
    else:
        for idx, row in bad.iterrows():
            with st.container(border=True):
                ...
                up_f = st.file_uploader("수리 사진 미리보기", type=['jpg','png'], key=f"img_{idx}")
                if up_f: st.image(up_f, width=250)
                
                if st.button("✅ 수리 완료 등록", key=f"btn_{idx}", type="primary"):
                    if sv and av:
...
# --- 마스터 관리 ---
    ...
    if not st.session_state.admin_authenticated:
        ...
        if st.button("🔓 관리 종료"): st.session_state.admin_authenticated = False; nav("조립 라인")
        m1, m2 = st.columns(2)
        with m1:
            with st.container(border=True):
        ...
        with m2:
            with st.container(border=True):
                st.subheader("계정 관리")
                nid, npw = st.text_input("ID"), st.text_input("PW", type="password")
                if st.button("계정 생성"):
