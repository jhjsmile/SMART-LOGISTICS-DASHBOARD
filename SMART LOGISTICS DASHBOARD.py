    ...
    if st.session_state.selected_cell != "전체 CELL":
        with st.container(border=True):
            ...
            with st.form("entry_gate_form"):
                ...
                target_item = f_c1.selectbox("품목 코드", g_items if target_model!="선택하세요." else ["대기"])
                target_sn = f_c2.text_input("S/N 입력")
                if st.form_submit_button("▶️ 생산 등록", use_container_width=True, type="primary"):
                    if target_model != "선택하세요." and target_sn:
                        db = st.session_state.production_db
                        if target_sn in db['시리얼'].values: st.error("중복 시리얼")
                        else:
...
# --- 7-2. 검사 / 포장 라인 ---
    ...
    with st.container(border=True):
        st.markdown(f"#### 📥 이전 공정({prev}) 완료 대기 물량")
...
# --- 7-3. 리포트 ---
    ...
    st.markdown("<h2 class='centered-title'>📊 실시간 생산 운영 리포트</h2>", unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        with cl:
            fig_b = px.bar(df.groupby('라인').size().reset_index(name='수량'), x='라인', y='수량', color='라인', title="공정 단계별 현황", template="plotly_white")
            st.plotly_chart(fig_b, use_container_width=True)
        with cr:
            fig_p = px.pie(df.groupby('모델').size().reset_index(name='수량'), values='수량', names='모델', hole=0.5, title="모델별 생산 비중")
            st.plotly_chart(fig_p, use_container_width=True)
        st.dataframe(df.sort_values('시간', ascending=False), use_container_width=True, hide_index=True)

# --- 7-4. 불량 공정 ---
elif curr_l == "불량 공정":
    st.markdown("<h2 class='centered-title'>🛠️ 불량 원인 분석 및 수리 조치</h2>", unsafe_allow_html=True)
    db = st.session_state.production_db
    wait = db[(db['반'] == curr_g) & (db['상태'] == "불량 처리 중")]
    
    if wait.empty: st.success("품질 이슈 없음")
    else:
        for idx, row in wait.iterrows():
            with st.container(border=True):
                ...
                img = st.file_uploader("증빙 사진 업로드", type=['jpg','png'], key=f"i_{idx}")
...
# --- 7-5. 수리 리포트 ---
    ...
    st.markdown("<h2 class='centered-title'>📈 품질 분석 및 수리 이력</h2>", unsafe_allow_html=True)
    db_hist = st.session_state.production_db
    hist_df = db_hist[db_hist['수리'] != ""]
    if not hist_df.empty:
        cl, cr = st.columns([1.5, 1])
        with cl:
            st.plotly_chart(px.bar(hist_df.groupby('라인').size().reset_index(name='수량'), x='라인', y='수량', title="공정별 불량 빈도"), use_container_width=True)
        with cr:
            st.plotly_chart(px.pie(hist_df.groupby('모델').size().reset_index(name='수량'), values='수량', names='모델', hole=0.4, title="모델별 불량 비중"), use_container_width=True)
        st.dataframe(hist_df, use_container_width=True, hide_index=True)
    else: st.info("기록된 내역 없음")
        ...
        for i, g_name in enumerate(NAV_GROUPS):
            with tabs[i]:
                ...
                with c1:
                    with st.container(border=True):
                        st.subheader("모델 등록")
                        nm = st.text_input(f"{g_name} 신규 모델", key=f"nm_{g_name}")
                ...
                with c2:
                    with st.container(border=True):
                        ...
                        if g_mods:
                            sm = st.selectbox(f"{g_name} 선택", g_mods, key=f"sm_{g_name}")
