...
# 4. 사이드바 내비게이션
# =================================================================
st.sidebar.title("🏭 생산 관리 시스템 v7.7")
st.sidebar.markdown("---")
...
if st.session_state.admin_page:
    ...
    if not st.session_state.is_authenticated:
        ...
        st.success("인증 성공: 마스터 데이터 제어 권한이 활성화되었습니다.")
        if st.button("관리자 로그아웃"):
            st.session_state.is_authenticated = False; st.rerun()

# =================================================================
# 6. 생산 통합 리포트 (요청대로 수정된 버전)
# =================================================================
...
# =================================================================
    ...
    with st.container(border=True):
        f1, f2 = st.columns(2)
        sel_m = f1.selectbox("모델 선택", ["선택하세요"] + st.session_state.master_models)
        if sel_m != "선택하세요":
            sel_i = f2.selectbox("품목 선택", ["전체"] + st.session_state.master_items_dict.get(sel_m, []))
            ready = st.session_state.production_db[(st.session_state.production_db['라인'] == "조립 라인") & (st.session_state.production_db['상태'] == "완료") & (st.session_state.production_db['모델'] == sel_m)]
            done_sns = st.session_state.production_db[st.session_state.production_db['라인'] == "검사 라인"]['시리얼'].unique()
