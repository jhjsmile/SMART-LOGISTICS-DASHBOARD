...
# =================================================================
st.set_page_config(page_title="생산 통합 관리 시스템 v8.6", layout="wide")
...
# =================================================================
# 3. 로그인 및 사이드바 내비게이션 (12, 13번 메뉴 배치)
# =================================================================
...
# 메뉴 배치 (12번: 리포트를 포장 아래에 / 13번: 수리 리포트를 수리 센터 아래에)
                ...
                if st.form_submit_button("▶️ 조립 등록", type="primary", use_container_width=True):
                    if m_choice != "선택하세요." and s_input:
                        ...
                        if not db[(db['시리얼'] == s_input) & (db['상태'] != "완료")].empty:
                            st.error(f"❌ 중복 오류: 시리얼 [{s_input}]은 이미 진행 중입니다.")
                        else:
...
# --- 5.2 품질 검사 / 5.3 출하 포장 ---
    ...
    with st.container(border=True):
        ...
        if sm != "선택하세요." and si != "품목을 선택하세요.":
            db = st.session_state.production_db
            # 이전 단계가 '완료'인 제품 중 현재 단계에 아직 안 들어온 것 추출
    ...
    if bad.empty: st.success("✅ 수리 대기 중인 불량 제품이 없습니다.")
        ...
        for idx, row in bad.iterrows():
            with st.container(border=True):
                ...
                s_val = c1.text_input("불량 원인", key=f"s_in_{idx}")
                a_val = c2.text_input("수리 조치", key=f"a_in_{idx}")
...
# --- 5.6 불량 수리 리포트 (13번 신규) ---
    ...
    if not rep_db.empty:
        c1, c2 = st.columns([3, 2])
        with c1:
            fig_r = px.bar(rep_db.groupby('라인').size().reset_index(name='수량'), x='라인', y='수량', title="공정별 불량 발생 건수")
...
# --- 5.7 마스터 관리 ---
    ...
    if not st.session_state.admin_authenticated:
        ...
        with m2:
            with st.container(border=True):
                st.subheader("품목 등록")
                target_m = st.selectbox("모델 선택", st.session_state.master_models)
                ni = st.text_input("품목코드")
                if st.button("품목 추가"):
                    if ni and ni not in st.session_state.master_items_dict[target_m]:
                        st.session_state.master_items_dict[target_m].append(ni); st.rerun()
        st.divider()
        st.markdown("<div class='section-title'>👥 계정 관리</div>", unsafe_allow_html=True)
