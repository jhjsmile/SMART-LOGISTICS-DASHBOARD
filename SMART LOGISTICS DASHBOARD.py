...
# =================================================================
# 4. 로그인 화면 및 사이드바 내비게이션 (v18.0 계층형 메뉴 개편)
# =================================================================
...
# --- [v18.0 추가] 제조 반 선택 시스템 ---
# 세션 상태에 선택된 반 정보를 저장합니다.
if 'selected_dept' not in st.session_state:
    st.session_state.selected_dept = "제조 1반"

# 상위 카테고리 선택 (제조 1반, 2반, 3반)
...
st.sidebar.markdown(f"#### 📍 {st.session_state.selected_dept} 메뉴")
...
# 그룹 1: 메인 공정 관리 (현재 선택된 '반'의 하위 메뉴로 동작)
for p in ["조립 라인", "검사 라인", "포장 라인", "리포트"]:
    if p in my_allowed:
        # 버튼 라벨에 '반' 정보를 포함하거나, 필터링 로직에 활용 가능
